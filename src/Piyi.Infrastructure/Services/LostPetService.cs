...
public sealed class LostPetService : ILostPetService
{
    ...
    public async Task<Result<IReadOnlyList<LostPetListItemResponse>>> GetActiveAsync(CancellationToken cancellationToken = default)
    {
        var items = await _dbContext.LostPets
            .AsNoTracking()
            .Include(x => x.Pet)
