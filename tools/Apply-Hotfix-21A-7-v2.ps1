...
Write-Host "Aplicando Piyí Hotfix 21A-7 v2 - Back global compatible..." -ForegroundColor Cyan
...
function Get-RelativeImportToBackButton($filePath) {
    $full = (Resolve-Path $filePath).Path.Replace('\','/')

    if ($full -match "/lib/src/features/([^/]+)/presentation/[^/]+\.dart$") {
        return "../../core/navigation/piyi_back_button.dart"
    }

    if ($full -match "/lib/src/features/([^/]+)/presentation/widgets/[^/]+\.dart$") {
        return "../../../core/navigation/piyi_back_button.dart"
    }
    ...
    # Ruta más común en este proyecto:
    return "../../../core/navigation/piyi_back_button.dart"
}
...
foreach ($file in $screenFiles) {
    ...
    # No duplicar si ya tiene leading explícito.
    if ($content -match "AppBar\s*\(\s*leading\s*:") {
        continue
    }

    # Agrega import relativo.
    if ($content -notmatch "piyi_back_button\.dart") {
        $relativeImport = Get-RelativeImportToBackButton $path
...
Write-Host ""
Write-Host "Hotfix 21A-7 v2 aplicado." -ForegroundColor Green
